...
st.set_page_config(
    page_title="CellDINO PCA Embeddings - Brieflow Analysis",
    layout="wide",
)
...
# (pca_embedding_analysis/scripts/run_celldino_pca.py), as opposed to the raw
# concatenated-embedding view on the "CellDINO Embeddings" page.

CELLDINO_PCA_OUTPUT_PATH = os.environ.get("CELLDINO_PCA_OUTPUT_PATH", "")
...
# pca_embedding_analysis/scripts/run_celldino_pca.py + pca_embedding_analysis/lib/rfp_filter.py).
RFP_FILTERS = {
    "All cells": "",
...
def tsv_path(level_prefix: str, rfp_suffix: str) -> str:
    return os.path.join(
        CELLDINO_PCA_OUTPUT_PATH, f"celldino{rfp_suffix}_{level_prefix}_pca_phate_leiden.tsv"
    )
...
def make_trace(x, y, marker, customdata, name: str):
    ...
    trace_cls = (
        go.Scatter if st.session_state.get("celldino_pca_vector_export", False) else go.Scattergl
    )
    return trace_cls(
        x=x,
        y=y,
...
def build_figure(df: pd.DataFrame) -> go.Figure:
    ...
    selected_item = st.session_state.get("celldino_pca_selected_cluster", None)
    selected_name = st.session_state.get("celldino_pca_selected_name", None)
    ...
    if st.session_state.get("celldino_pca_zoom_x") and st.session_state.get("celldino_pca_zoom_y"):
        fig.update_layout(
            xaxis=dict(range=st.session_state.celldino_pca_zoom_x),
            yaxis=dict(range=st.session_state.celldino_pca_zoom_y),
        )
...
def init_session_state():
    defaults = {
        "celldino_pca_selected_cluster": None,
        "celldino_pca_selected_name": None,
        "celldino_pca_zoom_x": None,
        "celldino_pca_zoom_y": None,
    }
...
def reset_selection():
    st.session_state.celldino_pca_selected_cluster = None
    st.session_state.celldino_pca_selected_name = None
    st.session_state.celldino_pca_zoom_x = None
    st.session_state.celldino_pca_zoom_y = None
...
st.title("CellDINO PCA Embedding Analysis")
st.caption(
...
if not CELLDINO_PCA_OUTPUT_PATH:
    st.error(
        "CELLDINO_PCA_OUTPUT_PATH environment variable is not set. "
        "Add it to 14.run_visualization.sh and restart the server."
    )
    st.stop()

with st.sidebar:
    st.markdown("## CellDINO PCA Controls")

    level = st.radio(
        "**Aggregation Level**",
        ["Per Gene", "Per Guide"],
        key="celldino_pca_level",
...
if raw.empty:
    st.warning(
        ...
        "Run scripts/run_celldino_pca.py (pca_embedding_analysis/) to generate this TSV."
    )
    st.stop()
...
with st.sidebar:
    ...
    current_name = st.session_state.celldino_pca_selected_name
    search_idx = search_options.index(current_name) if current_name in search_options else 0

    def on_name_search():
        chosen = st.session_state.celldino_pca_name_search
        if chosen == "— none —":
            st.session_state.celldino_pca_selected_name = None
            st.session_state.celldino_pca_selected_cluster = None
        else:
            st.session_state.celldino_pca_selected_name = chosen
            row = df[df["name"] == chosen]
            if not row.empty:
                st.session_state.celldino_pca_selected_cluster = str(row["cluster"].iloc[0])

    st.selectbox(
        "**Search gene / guide**",
        search_options,
        index=search_idx,
        key="celldino_pca_name_search",
    ...
    cur_cluster = st.session_state.celldino_pca_selected_cluster
    cluster_idx = cluster_options.index(cur_cluster) if cur_cluster in cluster_options else 0

    def on_cluster_select():
        chosen = st.session_state.celldino_pca_cluster_dropdown
        if chosen == "— none —":
            st.session_state.celldino_pca_selected_cluster = None
            st.session_state.celldino_pca_selected_name = None
        else:
            st.session_state.celldino_pca_selected_cluster = chosen
            genes_in_cluster = df[df["cluster"] == chosen]["name"].tolist()
            if genes_in_cluster:
                st.session_state.celldino_pca_selected_name = genes_in_cluster[0]

    st.selectbox(
        "**Select cluster**",
        cluster_options,
        index=cluster_idx,
        key="celldino_pca_cluster_dropdown",
...
st.checkbox(
    ...
    key="celldino_pca_vector_export",
)
fig = build_figure(df)
event = st.plotly_chart(
    fig,
    use_container_width=True,
    key="celldino_pca_plot",
    on_select="rerun",
...
if event.selection and event.selection.points:
    ...
    if "customdata" in pt and len(pt["customdata"]) > CLUSTER_IDX:
        ...
        if (
            st.session_state.celldino_pca_selected_cluster != clicked_cluster
            or st.session_state.celldino_pca_selected_name != clicked_name
        ):
            st.session_state.celldino_pca_selected_cluster = clicked_cluster
            st.session_state.celldino_pca_selected_name = clicked_name
            st.rerun()

selected_cluster = st.session_state.celldino_pca_selected_cluster
