...
st.set_page_config(
    page_title="CellDINO Embeddings - Brieflow Analysis",
    layout="wide",
)
...
CELLDINO_OUTPUT_PATH = os.environ.get("CELLDINO_OUTPUT_PATH", "")

# Channel combinations available for clustering. Each maps a human-readable label
# to the TSV suffix written by run_phate_leiden.py (ch<ids>), where the ids are
# fluorescence channel indices: 0=DAPI 1=CFP 2=YFP 3=RFP 4=Cy5.
CHANNEL_COMBOS = {
    "All (DAPI/CFP/YFP/RFP/Cy5)": "ch01234",
    "No CFP (DAPI/YFP/RFP/Cy5)": "ch0234",
    "CFP/RFP/Cy5": "ch134",
    "YFP/RFP/Cy5": "ch234",
    "RFP/Cy5": "ch34",
    "RFP only": "ch3",
...
# celldino_mp35_analysis/scripts/run_phate_leiden.py + pca_embedding_analysis/lib/rfp_filter.py).
RFP_FILTERS = {
    "All cells": "",
...
def tsv_path(level_prefix: str, combo_suffix: str, rfp_suffix: str) -> str:
    return os.path.join(
        CELLDINO_OUTPUT_PATH, f"{level_prefix}_phate_leiden_{combo_suffix}{rfp_suffix}.tsv"
    )
...
def prepare_df(raw: pd.DataFrame, name_col: str, leiden_res: int) -> pd.DataFrame:
    """Add a unified 'name' and 'cluster' column for the selected resolution."""
    df = raw.copy()
...
def make_trace(x, y, marker, customdata, name: str):
    ...
    trace_cls = (
        go.Scatter if st.session_state.get("celldino_vector_export", False) else go.Scattergl
    )
    return trace_cls(
        x=x,
        y=y,
...
def build_figure(df: pd.DataFrame) -> go.Figure:
    ...
    selected_item = st.session_state.get("celldino_selected_cluster", None)
    selected_name = st.session_state.get("celldino_selected_name", None)

    fig = go.Figure()

    if selected_item is not None:
        # Gray-out all unselected clusters
        ...
        # Selected cluster — split into highlighted name vs rest
        ...
        # No selection — all points with cluster colors
    ...
    # Restore saved zoom
    if st.session_state.get("celldino_zoom_x") and st.session_state.get("celldino_zoom_y"):
        fig.update_layout(
            xaxis=dict(range=st.session_state.celldino_zoom_x),
            yaxis=dict(range=st.session_state.celldino_zoom_y),
        )
...
def init_session_state():
    defaults = {
        "celldino_selected_cluster": None,
        "celldino_selected_name": None,
        "celldino_zoom_x": None,
        "celldino_zoom_y": None,
    }
...
def reset_selection():
    st.session_state.celldino_selected_cluster = None
    st.session_state.celldino_selected_name = None
    st.session_state.celldino_zoom_x = None
    st.session_state.celldino_zoom_y = None
...
st.title("CellDINO Embedding Analysis")

if not CELLDINO_OUTPUT_PATH:
    st.error(
        "CELLDINO_OUTPUT_PATH environment variable is not set. "
...
# ---- Sidebar controls ----
with st.sidebar:
    st.markdown("## CellDINO Controls")

    channel_combo = st.radio(
        "**Channel Combination**",
        list(CHANNEL_COMBOS.keys()),
        key="celldino_channel_combo",
    ...
    rfp_filter = st.radio(
        ...
        key="celldino_rfp_filter",
        on_change=reset_selection,
    )

# ---- Load data ----
name_col = get_name_col(level)
combo_suffix = CHANNEL_COMBOS[channel_combo]
rfp_suffix = RFP_FILTERS[rfp_filter]
level_prefix = "gene" if level == "Per Gene" else "guide"
current_tsv = tsv_path(level_prefix, combo_suffix, rfp_suffix)
raw = load_tsv(current_tsv)

if raw.empty:
    st.warning(
        f"No data found at `{current_tsv}`. "
        "Run scripts/run_phate_leiden.py to generate the TSV files for this channel combination / RFP filter."
    )
    st.stop()

leiden_col = f"leiden_{leiden_res}"
if leiden_col not in raw.columns:
    st.warning(f"Column `{leiden_col}` not found in TSV. Available: {[c for c in raw.columns if c.startswith('leiden_')]}")
    st.stop()
...
with st.sidebar:
    ...
    current_name = st.session_state.celldino_selected_name
    search_idx = search_options.index(current_name) if current_name in search_options else 0

    def on_name_search():
        chosen = st.session_state.celldino_name_search
        if chosen == "— none —":
            st.session_state.celldino_selected_name = None
            st.session_state.celldino_selected_cluster = None
        else:
            st.session_state.celldino_selected_name = chosen
            row = df[df["name"] == chosen]
            if not row.empty:
                st.session_state.celldino_selected_cluster = str(row["cluster"].iloc[0])

    st.selectbox(
        "**Search gene / guide**",
        search_options,
        index=search_idx,
        key="celldino_name_search",
        on_change=on_name_search,
    )

    # Cluster dropdown
    cluster_options = ["— none —"] + sorted(df["cluster"].unique(), key=lambda c: int(c) if c.isdigit() else -1)
    cur_cluster = st.session_state.celldino_selected_cluster
    cluster_idx = cluster_options.index(cur_cluster) if cur_cluster in cluster_options else 0

    def on_cluster_select():
        chosen = st.session_state.celldino_cluster_dropdown
        if chosen == "— none —":
            st.session_state.celldino_selected_cluster = None
            st.session_state.celldino_selected_name = None
        else:
            st.session_state.celldino_selected_cluster = chosen
            genes_in_cluster = df[df["cluster"] == chosen]["name"].tolist()
            if genes_in_cluster:
                st.session_state.celldino_selected_name = genes_in_cluster[0]

    st.selectbox(
        "**Select cluster**",
        cluster_options,
        index=cluster_idx,
        key="celldino_cluster_dropdown",
...
st.checkbox(
    ...
    key="celldino_vector_export",
)
fig = build_figure(df)
event = st.plotly_chart(fig, use_container_width=True, key="celldino_plot", on_select="rerun", config={"toImageButtonOptions": {"format": "svg"}})
...
if event.selection and event.selection.points:
    ...
    if "customdata" in pt and len(pt["customdata"]) > CLUSTER_IDX:
        ...
        if (
            st.session_state.celldino_selected_cluster != clicked_cluster
            or st.session_state.celldino_selected_name != clicked_name
        ):
            st.session_state.celldino_selected_cluster = clicked_cluster
            st.session_state.celldino_selected_name = clicked_name
            st.rerun()

# ---- Details panel ----
selected_cluster = st.session_state.celldino_selected_cluster
if selected_cluster is not None:
    ...
    st.markdown(f"### Cluster {selected_cluster} — {len(cluster_df)} perturbations ({pct_ctrl:.0f}% control)")

    display_cols = ["name", "gene_label", "cell_count", "is_control"] if level == "Per Guide" else ["name", "cell_count", "is_control"]
